#!/usr/bin/env bash
# run_bench.sh - build and run the benchmark kernels, print a CPI table.
#
...
printf '%-10s %10s %10s %7s %8s %8s %9s\n' \
       kernel cycles instret CPI stalls flushes bpred-acc
printf '%.0s-' {1..68}; echo
...
for k in "${KERNELS[@]}"; do
    ...
    printf '%-10s %10s %10s %7s %8s %8s %8s%%' \
        "$k" "$(get cycles "$perf")" "$(get instret "$perf")" "$(get CPI "$perf")" \
        "$(get stalls "$perf")" "$(get flushes "$perf")" "$(get accuracy "$bp")"
