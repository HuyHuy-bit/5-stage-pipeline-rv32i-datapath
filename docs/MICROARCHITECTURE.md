...
- **The I-cache and the backing memories still don't use Block RAM.** The D-cache data array does (4 × RAMB18, see Synthesis), but `icache.sv` kept its original array shape and `instr_mem.sv`/`data_mem.sv` still read combinationally, so the I-cache's ~8Kbit of storage is still costing ~10,000 flip-flops that a single BRAM tile would hold. The fix is mechanical now that the D-cache proves out the pattern — per-way flat arrays, one unified write port — and would likely take the I-cache config well under its current 51% LUT. Not done this pass simply because the I-cache already fits and the D-cache was the blocker.
- **Nothing here closes timing at a useful frequency.** ~76-79 MHz is a working number, not a good one, and the 2ns constraint used to derive it was deliberately unachievable rather than a target anyone tried to meet. No timing optimization has been attempted at all: no retiming, no pipelining of the tag-compare/way-select path, no attempt to shorten the redirect priority chain. The improvement plan's "attempt one timing optimization and measure the delta" step remains undone.
