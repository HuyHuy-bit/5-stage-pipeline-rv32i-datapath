...
- **The cache speedups above are CPI-only, and synthesis says that's the wrong number to trust as-is.** Real out-of-context synthesis on an Arty A7-35T (Vivado 2025.2): the core alone routes at ~79MHz, the 1KB I-cache at ~77MHz — and the 4KB D-cache **doesn't fit the device at all**, 3.2x over its LUT budget, because none of the backing memories or cache arrays register their reads (so nothing infers Block RAM; it all lands in flip-flops and LUTs instead). fmax-adjusted, the I-cache config is already *slower* than no cache at all before the D-cache's fit failure even enters the picture. The CPI table above is real and reproducible, but treat every speedup in it as contingent on a synthesis rework (registered cache reads) that hasn't happened yet — full details and the per-config numbers are in the Synthesis section of the microarchitecture doc.
